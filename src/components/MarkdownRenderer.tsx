...
import { oneDark, materialLight } from "react-syntax-highlighter/dist/esm/styles/prism" // 高亮主题
    ...
    <div className="code-block">
      <div className="flex justify-between py-2 px-4 rounded-t-xl bg-[#f2f2fe] dark:bg-gray-800 ">
        ...
        <button
          ...
          aria-label="复制代码"
        >
          <IconFont iconClass="iconfont icon-fuzhi" color={copied ? "#3498db" : "gray"} size={16} />
        </button>
      </div>
      {/* 代码高亮 */}
      <SyntaxHighlighter
...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content }) => {
  return (
    <ReactMarkdown
      remarkPlugins={[remarkGfm, [rehypeExternalLinks, { target: "_blank", rel: ["nofollow"] }]]}
      ...
      components={{
        ...
        ul: ({ node, depth, ...props }: any) => (
          <ul
            className={`list-disc pl-4 sm:pl-6 mb-4 space-y-1 sm:space-y-2 text-gray-700 dark:text-gray-300 break-words ${
        ...
        ol: ({ node, depth, ...props }: any) => (
          <ol
            className={`list-decimal pl-4 sm:pl-6 mb-4 space-y-1 sm:space-y-2 text-gray-700 dark:text-gray-300 break-words ${
              depth > 0 ? "ml-2 sm:ml-4" : ""
            }`}
            {...props}
          />
        ...
        pre: ({ node, ...props }) => (
          <pre className="m-1  max-w-full overflow-x-auto overflow-y-hidden" {...props} />
        ),
